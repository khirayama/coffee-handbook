...
function xmlUrlTemplate(page: IPage): string {
  return `<url><loc>${page.url}</loc>${page.lastmod ? `<lastmod>${page.lastmod}</lastmod>` : ''}${page.freq ? `<changefreq>${page.freq}</changefreq>` : ''}${page.priority ? `<priority>${page.priority}</priority>` : ''}</url>`;
}

function xmlSitemapTemplate(urlTemplates: string[]): string {
  return `<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${urlTemplates.join('')}</urlset>`;
}
